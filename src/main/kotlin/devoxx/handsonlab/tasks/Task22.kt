 ...
 */
fun CoroutineScope.produceNumbers(min: Int, max: Int): ReceiveChannel<Int> =
 ...
 */
fun CoroutineScope.filterNumbers(input: ReceiveChannel<Int>): ReceiveChannel<Int> =
    produce(Dispatchers.Default) {
        input.filter { it % 2 == 0 }
            .consumeEach {
                send(it)
 ...
 */
fun CoroutineScope.mapNumbersToStrings(input: ReceiveChannel<Int>): ReceiveChannel<String> =
    produce(Dispatchers.Default) {
        input.consumeEach {
                send(it.toString())
            }
